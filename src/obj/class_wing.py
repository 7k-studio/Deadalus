...
class Wing:
    ...
    def move(self, cmp_X:float, cmp_Y:float, cmp_Z:float, wng_X:float, wng_Y:float, wng_Z:float, seg_X:float, seg_Y:float, seg_Z:float):
        ...
        self.logger.debug(f"> Moving WING geometry by X:{move_X}, Y:{move_Y}, Z:{move_Z}...")
    ...
    def scale(self, scale:float):
        ...
        self.logger.debug(f"> Scaling WING geometry by {scale}...")
    ...
    def rotate(self, incidence:float):

        self.logger.debug(f"> Rotating WING geometry by {incidence} degrees...")

        theta = np.radians(float(incidence))
    ...
    def build_connection(self):
        ...
        if len(self.segments) > 1:
            for i in range(len(self.segments)-1):
                ...
                # following control points in Z(width) direction
                ...
                if self.segments[i].anchor == 'G1' and self.segments[i+1].anchor == 'G0':
                    ...
                    child_le_ps_tan_cp = np.array([child_le_ps_seg_anchor[0] , child_le_ps_seg_anchor[1], (child_le_ps_seg_anchor[2] + 0)])
                    child_le_ss_tan_cp = np.array([child_le_ss_seg_anchor[0], child_le_ss_seg_anchor[1], (child_le_ss_seg_anchor[2] + 0)])
                    child_te_ps_tan_cp = np.array([child_te_ps_seg_anchor[0], child_te_ps_seg_anchor[1], (child_te_ps_seg_anchor[2] + 0)])
                    child_te_ss_tan_cp = np.array([child_te_ss_seg_anchor[0] , child_te_ss_seg_anchor[1], (child_te_ss_seg_anchor[2] + 0)])
                ...
                self.segments[i].SS.control_points['v_fwd'] = self.segments[i].control_points['ss']
                self.segments[i].SS.control_points['v_rwd'] = self.segments[i+1].control_points['ss']
                ...
                self.segments[i].LE.control_points['v_fwd'] = self.segments[i].control_points['le']
                self.segments[i].LE.control_points['v_rwd'] = self.segments[i+1].control_points['le']
                ...
                self.segments[i].TE.control_points['v_fwd'] = self.segments[i].control_points['te']
                self.segments[i].TE.control_points['v_rwd'] = self.segments[i+1].control_points['te']
                ...
                #self.logger.debug('Grid or surf: ', surf)
                #self.build_b_spline_surf()
...
def make_nurbs_surface_points(control_points, resolution=100):
    samples_u=int(resolution/3+7)
    samples_v=int(resolution/3+7)
