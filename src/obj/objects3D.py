...
class Surface:
    ...
    def build_surface_mesh(self, surf):
        """
    ...
    def make_surface(self, control_grid):
        """
        Create a NURBS surface from a 2D grid of control points.
        control_grid: list of rows (u-direction), each row is list of [x,y,z] points
        """
    ...
    def resample_curve(self, points, n_out):
    ...
    def sample_curve(self, points, t):
        """
    ...
    def make_nurbs_surface_points(self, control_points):
    ...
    def make_uv_grid_from_boundaries(self):
        """
        ...
        """
        u_count = len(self.control_points['u_fwd'][0])
        v_count = len(self.control_points['v_fwd'][0])

        # Build the control points grid
        grid_control_points = np.zeros((u_count, v_count, 3))  # (u, v, xyz)
        ...
        for u in range(u_count):
            for v in range(v_count):
                ...
                cu = (1 - tu) * np.array([self.control_points['v_fwd'][0][v], self.control_points['v_fwd'][1][v], self.control_points['v_fwd'][2][v]]) + tu * np.array([self.control_points['v_rwd'][0][v], self.control_points['v_rwd'][1][v], self.control_points['v_rwd'][2][v]])
                # v-direction curve at u
                cv = (1 - tv) * np.array([self.control_points['u_fwd'][0][u], self.control_points['u_fwd'][1][u], self.control_points['u_fwd'][2][u]]) + tv * np.array([self.control_points['u_rwd'][0][u], self.control_points['u_rwd'][1][u], self.control_points['u_rwd'][2][u]])
                # corners
                p00 = np.array([self.control_points['u_fwd'][0][0], self.control_points['u_fwd'][1][0], self.control_points['u_fwd'][2][0]])
                p10 = np.array([self.control_points['u_fwd'][0][u_count-1], self.control_points['u_fwd'][1][u_count-1], self.control_points['u_fwd'][2][u_count-1]])
                p01 = np.array([self.control_points['u_rwd'][0][0], self.control_points['u_rwd'][1][0], self.control_points['u_rwd'][2][0]])
                p11 = np.array([self.control_points['u_rwd'][0][u_count-1], self.control_points['u_rwd'][1][u_count-1], self.control_points['u_rwd'][2][u_count-1]])
                bilinear = (1 - tu) * (1 - tv) * p00 + tu * (1 - tv) * p10 + (1 - tu) * tv * p01 + tu * tv * p11
                grid_control_points[u, v, :] = cu + cv - bilinear

        # Optionally, print or use the grid
        print(f"Control points grid ({u_count}x{v_count}x3) established")
        #print(control_points)

        self.uv_grid = grid_control_points

    def build_b_spline_surf_for_segment(self, segment):
        """
