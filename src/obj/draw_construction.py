...
p_le_start = [airfoil.params['origin_X'], airfoil.params['origin_Y']+airfoil.LE.params['offset']]
p_le_end = tools.vec_translate(p_le_start, airfoil.LE.params['depth'], airfoil.LE.params['angle'])

a0 = math.tan(np.radians(90+airfoil.LE.params['angle'])) # Directional param of the leading edge straight and the pararel one
a1 = math.tan(np.radians(airfoil.PS.params['fwd_angle']+airfoil.LE.params['angle'])) # Directional param of the pressure side forward slope
a2 = math.tan(np.radians(airfoil.SS.params['fwd_angle']+airfoil.LE.params['angle'])) # Directional param of the suction side forward slope
...
b1 = p_le_end[1]+(airfoil.LE.params['thickness']/2*math.cos(np.radians(airfoil.LE.params['angle'])))-a1*(p_le_end[0]-airfoil.LE.params['thickness']/2*math.sin(np.radians(airfoil.LE.params['angle'])))
b2 = p_le_end[1]-(airfoil.LE.params['thickness']/2*math.cos(np.radians(airfoil.LE.params['angle'])))-a2*(p_le_end[0]+airfoil.LE.params['thickness']/2*math.sin(np.radians(airfoil.LE.params['angle'])))
...
p_te_start = tools.vec_translate(p_le_start, airfoil.params['chord'], airfoil.params["incidence"])

p_te_start = [p_te_start[0], p_te_start[1]+airfoil.TE.params['offset']]
p_te_end = [p_te_start[0]-airfoil.TE.params['depth']*math.cos(np.radians(airfoil.TE.params['angle'])), p_te_start[1]-(airfoil.TE.params['depth']*math.sin(np.radians(airfoil.TE.params['angle'])))]
a3 = math.tan(np.radians(90+airfoil.TE.params['angle']))
a4 = math.tan(np.radians(airfoil.PS.params['rwd_angle']+airfoil.TE.params['angle']))
...
p_ps_le = p_le_ps
p_ps_te = p_te_ps
p_ps_1 = [airfoil.params['origin_X']+airfoil.PS.params['fwd_accel'], a1*(airfoil.params['origin_X']+airfoil.PS.params['fwd_accel'])+b1]
p_ps_2 = [p_ps_te[0]-airfoil.PS.params['rwd_accel'], a4*(p_ps_te[0]-airfoil.PS.params['rwd_accel'])+b4]   
...
plt.plot(p_le_end[0], p_le_end[1], 'x', color='black', label='Leading Edge End')
plt.plot(p_le_t[0], p_le_t[1], 'x', color='black', label='Leading Edge End')
plt.plot(p_le_d[0], p_le_d[1], 'x', color='black', label='Leading Edge End')

plt.plot(p_te_start[0], p_te_start[1], 'o', color='black', label='Trailing Edge Start')
