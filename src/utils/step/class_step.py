...
class Organization:
    def __init__(self, idx):
        self.idx = idx
        self.organization = 'UNSPECIFIED'
    
    def export(self):
        return f"#{self.idx} = ORGANIZATION ( '{self.organization}', '{self.organization}', '' ) ;"
...
class Person:
    ...
    def export(self):
        return f"#{self.idx} = PERSON ( '{self.name}', '{self.name}', '{self.name}', ('{self.name}'), ('{self.name}'), ('{self.name}') ) ;"
        
class PersonAndOrganization:
    def __init__(self, idx, person, organization):
        self.idx = idx
        self.person = person
        self.organization = organization
    
    def export(self):
        return f"#{self.idx} = PERSON_AND_ORGANIZATION ( #{self.person.idx}, #{self.organization.idx} ) ;"
...
class ShellBasedSurfaceModel:
    def __init__(self, idx, desc=None, shell_list=None):
        self.idx = idx
        self.desc = desc if desc != None else 'NONE'
        self.open_shells = shell_list
    
    def export(self):
        open_shells = ', '.join([f'#{obj.idx}' for obj in self.open_shells])
        return f"#{self.idx} = SHELL_BASED_SURFACE_MODEL ( '{self.desc}', ( {open_shells} ) ) ;"

class SecurityClassification:
    def __init__(self, idx, clasification_level):
        self.idx = idx
        self.clasification_level = clasification_level
    
    def export(self):
        return f"#{self.idx} = SECURITY_CLASSIFICATION ( '', '', #{self.clasification_level.idx} ) ;"

class SecurityClassificationLevel:
    def __init__(self, idx, level='unclassified'):
        self.idx = idx
        self.level = level
    
    def export(self):
        return f"#{self.idx} = SECURITY_CLASSIFICATION_LEVEL ( '{self.level}' ) ;"
